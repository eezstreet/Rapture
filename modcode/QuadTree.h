...
public:
	bool				Contains(QuadTree* child, C* node);
	void				Clear();

	QuadTree(T x1, T y1, T w1, T h1, unsigned d1, unsigned m1) :
		x(x1),
		y(y1),
		w(w1),
		h(h1),
		...
		maxDepth(m1) {
			...
			NW = new QuadTree<T, C>(x, y, w / (T)2, h / (T)2, depth+1, maxDepth);
			NE = new QuadTree<T, C>(x + w / (T)2, y, w / (T)2, h / (T)2, depth+1, maxDepth);
			SW = new QuadTree<T, C>(x, y + h / (T)2, w / (T)2, h / (T)2, depth+1, maxDepth);
			SE = new QuadTree<T, C>(x + w / (T)2, y + h / (T)2, w / (T)2, h / (T)2, depth+1, maxDepth);
	}
	...
	bool Contains(const QuadTree* child, const C* node) {
	...
	vector<C*> NodesAt(const T posX, const T posY) {
		if(depth == maxDepth) {
			return nodes;
		}
